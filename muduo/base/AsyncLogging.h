...
namespace muduo
{
    ...
    private:
        ...
        // 可理解为Buffer的智能指针, 能管理Buffer的生存期, 类似于C++11中的unique_ptr, 具备移动语义
        // 两个unique_ptr不能指向一个对象，不能进行复制操作只能进行移动操作

        string basename_;         // 日志文件名
        size_t rollSize_;         // 日志文件的滚动大小
        const int flushInterval_; // 超时时间, 单位秒, 在 flushInterval_ 内, 缓冲区没写满, 仍将缓冲区中的数据写到文件中
