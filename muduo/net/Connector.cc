...
void Connector::stopInLoop()
{
    ...
    if (state_ == kConnecting)
    {
        ...
        retry(sockfd);                        // 这里并非要重连, 只是调用close了cfd.
    }
}
...
void Connector::handleWrite()
{
    ...
    if (state_ == kConnecting)
    {
        int sockfd = removeAndResetChannel(); // 从poller中移除关注, 并将channel置空. 防止一直触发可写事件.

        // socket可写并不意味着连接一定建立成功, 还需要用 getsockopt(sockfd, SOL_SOCKET, SO_ERROR, ...) 再次确认一下
